...
export function CustomMoviesAdmin() {
  ...
  const [movies, setMovies] = useLocalStorage<CustomMovie[]>(
    CUSTOM_MOVIES_KEY,
    initialCustomMovies,
  );

  const [bin, setBin] = useLocalStorage<CustomMovie[]>(
    CUSTOM_MOVIE_BIN_KEY,
    [],
  );
  ...
  const [featuredMovies, setFeaturedMovies] = useState<CustomMovie[]>(
    () => readFeaturedMovies() as CustomMovie[],
  );
  ...
  const toggleFeatured = (movie: CustomMovie) => {
    const nextFeaturedMovies = featuredMovies.some(
  ...
  const toggleAllFeatured = () => {
    ...
    const allMoviesAreFeatured =
      movies.length > 0 &&
      movies.every((movie) =>
        featuredMovies.some((item) => item.id === movie.id),
      );
    ...
    <>
      <div className="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
        ...
        <div className="d-flex flex-wrap gap-2">
          ...
          >
            {movies.length > 0 &&
            movies.every((movie) => movie.isHidden) ? (
              <FaEye />
            ) : (
              <FaEyeSlash />
            )}
            ...
            <tr key={movie.id}>
              ...
              <td>
                <Button
                  ...
                  className={
                    featuredMovies.some(
                      ...
                      ? "admin-featured-button active me-2"
                      : "admin-featured-button me-2"
                  }
                  onClick={() => toggleFeatured(movie)}
                  aria-label={`Destacar ${movie.title}`}
                >
                  <FaStar />
                </Button>
                ...
                <Button
                  ...
                  aria-label={
                    movie.isHidden
                      ? `Mostrar ${movie.title} en el catálogo`
                      : `Ocultar ${movie.title} del catálogo`
                  }
                >
                  {movie.isHidden ? <FaEye /> : <FaEyeSlash />}
      ...
      {!movies.length && (
        <p className="text-muted">
          No hay películas personalizadas cargadas.
        </p>
      )}
          ...
          {bin.map((movie) => (
            <tr key={movie.id}>
              ...
              <td className="text-end">
                <Button
                  size="sm"
                  className="admin-restore-button me-2"
                  onClick={() => restore(movie)}
                >
                  <FaUndo />
                  Restaurar
                </Button>
                ...
                <Button
                  size="sm"
                  className="admin-delete-button"
                  aria-label={`Eliminar ${movie.title} definitivamente`}
      ...
      >
        ...
        >
          <Modal.Title>¿Limpiar películas?</Modal.Title>
        </Modal.Header>

        <Modal.Body>
          Las películas se moverán a la papelera y podrán restaurarse.
        </Modal.Body>
